package net.darkhax.tesla.lib;

public class Constants {
    
    public static final String MOD_ID = "tesla";
    public static final String MOD_NAME = "TESLA";
    public static final String VERSION_NUMBER = "1.0.0.0";
    public static final String MCVERSION = "[1.9,1.9.10]";
    public static final String CLIENT_PROXY_CLASS = "net.darkhax.tesla.client.ClientProxy";
    public static final String SERVER_PROXY_CLASS = "net.darkhax.tesla.common.CommonProxy";
}
